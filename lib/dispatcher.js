...
(function (__dirname, module) {
    ...
    function listen(app, logger, secure, hidden) {
        ...
        io.configure(function () {
            io.set('log level', 1);
            console.log("hidden is: " + JSON.stringify(hidden));
            if (hidden.redis) {
                console.log("hidden.redis=" + JSON.stringify(hidden.redis));
                var store = new RedisStore({
                    redisClient:hidden.redis,
                    redisPub:hidden.redis,
                    redisSub:hidden.redis
                });
